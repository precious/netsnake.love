...
function rcvCallback(data, ip, port)
	...
	if datafromclient[2].data:sub(1,1) == "a" then
...
function update(dt)
 ...
 y_cord_client1 = y_cord_client1+5*b_client1
	    server:send ("px_s" .. bin:pack(pieces_x) .. "py_s" .. bin:pack(pieces_y) .. "px_c1" .. bin:pack(pieces_x_client1) .. "py_c1" .. bin:pack(pieces_y_client1) .. "j_s" .. tonumber(j) .. "j_c1" .. tonumber(j_client1))
-- 	server:send("x_s" .. tostring(x_cord) .. "y_s" .. tostring(y_cord) .. "x_c1" .. tostring(x_cord_client1) .. "y_c1" .. tostring(y_cord_client1))
 
  j = j +1		-- Changing of the current piece
  if j>count then j = 1 end
 pieces_x[j] = x_cord
 pieces_y[j] = y_cord

  j_client1 = j_client1 +1		-- Changing of the current piece
  if j_client1>count then j_client1 = 1 end
 pieces_x_client1[j] = x_cord_client1
 pieces_y_client1[j] = y_cord_client1

    id = isbordercrossed(x_cord, y_cord)
      if id~=0 then x_cord, y_cord = correctit(x_cord, y_cord,id) end
    ...
    if ok then
         repeat
           ...
           messes_y[1] = math.random(9,love.graphics.getHeight() - 9)
    ...
    issnakecrosseditself(pieces_x,pieces_y)
    issnakecrosseditself(pieces_x_client1,pieces_y_client1)
...
function isbordercrossed(x,y) 	 -- Is the border crossed?
   if x > love.graphics.getWidth() then return 1
     elseif x < 0 then return 2
     elseif y > love.graphics.getHeight() then return 3
     elseif y < 0 then return 4
   end
 return 0
end
...
function correctit(x,y,id)
   if id == 1 then x = 0
     elseif id == 2 then x = love.graphics.getWidth()
     elseif id == 3 then y = 0
     elseif id == 4 then y = love.graphics.getHeight()
   end
 return x,y
end

function issnakecrosseditself(table_x,table_y)	-- If the snake has crossed itself then restart the game
 for i = 1,count-1 do			
   for j = i+1,count do
